...
export class SquadsEmbeddedWalletAdapter extends BaseWalletAdapter {
    ...
    constructor(targetOrigin: string = "https://v3.squads.so/") {
        super();
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            const vaultInfo = await this._messageBus.sendRequest(
                `getVaultInfo#${Date.now()}`,
                "getVaultInfo"
            );
            console.log("Received request for vault info");
